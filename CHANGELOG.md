...
### Added
- Community guidelines for contributing, conduct, supported security versions, secure operation, and vulnerability reporting
...
### Added
- **Inferred match scope highlighting** — inferred matches now show both model and target scope paths side-by-side with differing segments highlighted (brackets in terminal, `<mark>` in HTML)
- `InferredMatch` struct pairs model + target role assignments for downstream consumption (JSON: `{"model": {...}, "target": {...}}`)
- Inferred Matches section added to model compare detail view (terminal + HTML)
